...
public final class MaxflashImporter extends Importer {
	...
	private final static int ATR_MAX_MIN_FILE_SIZE = AtrFile.HEADER_SIZE + 0x13 * AtrFile.SECTOR_SIZE_SD + BANK_SIZE;
	private final static int ATR_MAX_MAX_FILE_SIZE = 2 * MB;

	private static final String BUILD_PREFIX = "ATARIMAX Flash Utility Build ";
	private static final byte[] BUILD_PREFIX_BYTES = ASCIIString.getBytes(BUILD_PREFIX);
	...
	private void addBuildInfo(String signature, int signatureOffset, int mode, int modeOffset) {
		...
		if (signatureOffset < 0) {
			throw new IllegalArgumentException(
					"Parameter 'signatureOffset' must not be negative. Specified value was " + signatureOffset + ".");
		}
		buildInfos.add(new BuildInfo(signature, signatureOffset, mode, modeOffset));
	}

	private void determineCartridgeTypeAndBuild(AtrProgrammingFile atrProgrammingFile) {
		...
		for (int i = 0; i < buildInfos.size() && buildInfo == null; i++) {
			...
			if (AtrProgrammingFile.containsStringAt(atrProgrammingFile.getBootCode(), currentBuildInfo.signature,
		...
		if (buildInfo == null) {
			...
			int signatureOffset = ByteArrayUtility.getIndexOf(atrProgrammingFile.getBootCode(), 0, BANK_SIZE - 1,
					BUILD_PREFIX_BYTES);
			...
			if (signatureOffset >= 0) {
				...
				buildInfo = new BuildInfo(signature, signatureOffset, MODE_UNKNOWN_SIGNATURE, 0);
			}
		}
	...
	private byte[] createCartridgeContent(AtrProgrammingFile atrProgrammingFile) throws CoreException {
		if (atrProgrammingFile == null) {
			throw new IllegalArgumentException("Parameter 'programmingImage' must not be null.");
		}
		...
		Log.logInfo("Processing {0}: {1}",
				new Object[] { atrProgrammingFile.getSourceFilePath(), buildInfo.toString() });
		...
		case MODE_FULL_IMAGE:
			...
			System.arraycopy(atrProgrammingFile.getSourceFileContent(), sourceFileOffset, cartridgeContent, 0,
		...
		case MODE_USAGE_MAP:
			...
			int bootCodeUsageMapOffset = buildInfo.modeOffset - AtrFile.HEADER_SIZE;
			...
			for (int i = 0; i < banksCount; i++) {
				...
				if (usage == 0) {
					// Yes, copy used bank.
					System.arraycopy(atrProgrammingFile.getSourceFileContent(), sourceFileContentOffset,
							cartridgeContent, cartridgeContentOffset, BANK_SIZE);
					sourceFileContentOffset += BANK_SIZE;
				} else if (usage == (byte) 0xff) {
					// No, fill unused banks with 0xff.
					Arrays.fill(cartridgeContent, cartridgeContentOffset, cartridgeContentOffset + BANK_SIZE,
							(byte) 0xff);
				} else {
					// ERROR: File '{0}' cannot be converted because it
					// contains the invalid usage tab entry {1} at offset
					// {2}.
					throw new CoreException(Messages.E503, TextUtility.formatAsHexaDecimal(usage, 256),
							TextUtility.formatAsHexaDecimal(bootCodeUsageMapOffset, bootCodeUsageMapOffset),
	...
	public void importFile(Workbook workbook, File file, ImportResult result) {
		...
		if (!flashTargetType.isContentTypeSupported(ContentType.CARTRIDGE_ATMAX_128)
				&& !flashTargetType.isContentTypeSupported(ContentType.CARTRIDGE_ATMAX_1024)) {
			return;
		}

		String fileName = file.getName();
		long fileLength = file.length();
		boolean relevant = (fileName.toLowerCase().endsWith(FileExtensions.ATR_IMAGE)
				&& (fileLength > MaxflashImporter.ATR_MAX_MIN_FILE_SIZE
						&& fileLength < MaxflashImporter.ATR_MAX_MAX_FILE_SIZE));
		if (!relevant) {
			return;
		}
		...
		File targetFile = new File(sourceFile.getParentFile(),
				fileName.substring(0, fileName.length() - FileExtensions.ATR_IMAGE.length())
		...
		try {
			...
			try {
				...
				byte[] cartridgeHeader = CartridgeFileUtility.createCartridgeHeaderWithCheckSum(
						atrProgrammingFile.getCartridgeType().getNumericId(), cartridgeContent);

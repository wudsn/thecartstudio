...
public final class WorkbookEntry implements Comparable<WorkbookEntry> {
	...
	WorkbookEntryType type = Attributes.TYPE.deserializeValueSet(attributes,
	...
	setFavoriteIndicator(Attributes.FAVORITE_INDICATOR
		.deserializeBoolean(attributes, favoriteIndicator));
	...
	ContentType contentType = Attributes.CONTENT_TYPE
		.deserializeValueSet(attributes, ContentType.class);
	if (contentType != null) {
	...
	setStartBankNumber(
		Attributes.START_BANK.deserializeInteger(attributes));
	setRequiredBanksCount(
		Attributes.REQUIRED_BANKS_COUNT.deserializeInteger(attributes));
    }

...
public final class WorkbookEntry implements Comparable<WorkbookEntry> {
	...
	public static final class Attributes {
		...
		public static final Attribute FILE_HEADER_TYPE = new Attribute("fileHeaderType",
				DataTypes.WorkbookEntry_FileHeaderType);
		public static final Attribute CONTENT_SIZE = new Attribute("contentSize", DataTypes.WorkbookEntry_ContentSize);
		public static final Attribute CONTENT_CRC32 = new Attribute("contentCRC32",
				DataTypes.WorkbookEntry_ContentCRC32);
		...
		public static final Attribute START_BANK_FIXED_INDICATOR = new Attribute("startBankFixedIndicator",
				DataTypes.WorkbookEntry_StartBankFixedIndicator);
		public static final Attribute START_BANK = new Attribute("startBankNumber",
				DataTypes.WorkbookEntry_StartBankNumber);
		public static final Attribute REQUIRED_BANKS_COUNT = new Attribute("requiredBanksCount",
				DataTypes.WorkbookEntry_RequiredBanksCount);
		public static final Attribute BANKS_ASSIGNED_INDICATOR = new Attribute("banksAssignedIndicator",
	...
	public static final class Parameter implements Comparable<Parameter> {
		...
		public Parameter(int key, int value) {
			this("$" + HexUtility.getLongValueHexString(key), "$" + HexUtility.getLongValueHexString(value));
		}
		...
		/**
		 * Determine if the specified value is a valid, non-negative integer in hex
		 * notation.
		 * 
		 * @param value
		 *            The value, may be empty, not <code>null</code>.
		 * @return <code>true</code> if the specified value is a valid, non-negative
		 *         integer in hex notation, <code>false</code> otherwise.
		 */
		...
		private boolean isValid() {
			...
			boolean directPatch = isKeyInteger() && isValueInteger() && getValueAsInteger() < 256;
	...
	/**
	 * Gets the file content size in bytes, i.e. the file size without the header
	 * size.
	 * 
	...
	/**
	 * Gets the number of banks required as alignment. Alignment is performed in way
	 * that the result is always a power of 2.
	 * 
	 * @return The number of banks required as alignment. For unknown and plain file
	 *         entries, the alignment is a single bank, no matter what the bank size
	 *         is.
	 */
	public int getAlignmentBanksCount() {
		if (!alignmentBanksCountValid) {
			if (contentType.isAligmentRequired()) {
				...
				if (alignmentBanksCount != Integer.lowestOneBit(requiredBanksCount)) {
	...
	final void serialize(Element element) {
		...
		Attributes.CONTENT_CRC32.serializeLongAsHex(element, getContentCRC32() & 0xffffffffl);
		...
		Attributes.START_BANK_FIXED_INDICATOR.serializeBoolean(element, isStartBankFixed());
		Attributes.START_BANK.serializeInteger(element, getStartBankNumber());
		Attributes.REQUIRED_BANKS_COUNT.serializeInteger(element, getRequiredBanksCount());
	}
	...
	final void deserialize(org.xml.sax.Attributes attributes) throws SAXException {
		...
		FileHeaderType fileHeaderType = Attributes.FILE_HEADER_TYPE.deserializeValueSet(attributes,
				FileHeaderType.class);
		if (fileHeaderType != null) {
			setFileHeaderType(fileHeaderType);
		}

		setContentCRC32((int) Attributes.CONTENT_CRC32.deserializeLongAsHex(attributes));
		...
		setStartBankFixedIndicator(
				Attributes.START_BANK_FIXED_INDICATOR.deserializeBoolean(attributes, startBankFixedIndicator));
		setStartBankNumber(Attributes.START_BANK.deserializeInteger(attributes));
		setRequiredBanksCount(Attributes.REQUIRED_BANKS_COUNT.deserializeInteger(attributes));
	}

	@Override
	public String toString() {
		return "type=" + type + " title=" + title + " filePath=" + filePath + " contentType=" + contentType
				+ " fileSize=" + fileSize + " displayMode=" + displayMode + " startBankFixedIndicator="
				+ startBankFixedIndicator + " startBankNumber=" + startBankNumber + " requiredBanksCount="
				+ requiredBanksCount + " banksAssigned=" + banksAssigned;
	}
	...
	public boolean equals(WorkbookEntry other) {
		...
		return other.type.equals(type) && other.title.equals(title) && other.genreName.equals(genreName)
				&& other.filePath.equals(filePath) && other.fileSize == fileSize
				&& other.contentType.equals(contentType) && other.displayMode.equals(displayMode)
				&& other.startBankNumber == startBankNumber && other.requiredBanksCount == requiredBanksCount;
	}

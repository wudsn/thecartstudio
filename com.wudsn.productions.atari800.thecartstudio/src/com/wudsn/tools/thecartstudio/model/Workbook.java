...
public final class Workbook {
	...
	private static final class XMLHandler extends com.wudsn.tools.base.common.XMLHandler {
		...
		public void startSave(Document document, MessageQueue messageQueue) {
			...
			// Root elements
			Element workbookElement = document.createElement(WorkbookRoot.Attributes.ELEMENT_NAME);
			...
			for (WorkbookGenre entry : root.getUnmodifiableGenresList()) {
				Element entryElement = document.createElement(WorkbookGenre.Attributes.ELEMENT_NAME);
			...
			for (WorkbookEntry entry : root.getUnmodifiableEntriesList()) {
				Element entryElement = document.createElement(WorkbookEntry.Attributes.ELEMENT_NAME);
				workbookElement.appendChild(entryElement);
				entry.serialize(entryElement);
			}
		...
		try {
			...
			for (int i = 0; i < filesList.size() && !callback.isCancelled(); i++) {
				...
				try {
					...
					int number = i + 1;
					if (number % steps == 0) {
						messageQueue.sendMessage(entryFile, WorkbookEntry.Attributes.FILE_PATH, Messages.S416,
								TextUtility.formatAsDecimal(number), TextUtility.formatAsDecimal(totalNumber),
								TextUtility.formatAsDecimalPercent(number, totalNumber));
					}

					WorkbookEntry entry = addEntry(entryFile, callback, messageQueue);
					if (entry != null) {
						firstAddedEntry = entry;
					}
				} catch (Throwable ex) {
					throw new RuntimeException("Exception while adding " + entryFile.getName(), ex);
				}
			}
	...
	/**
	 * Adds a new entry based on a file. Atomic option, does not fire mode change.
	...
	private WorkbookEntry addEntry(File file, WorkbookAddEntriesCallback callback, MessageQueue messageQueue) {
		...
		ImportResult importResult = Importer.autoConvertFile(this, file, messageQueue);
		if (importResult.convertedFileException != null) {
			if (importResult.convertedFileException.getSeverity() == Message.ERROR) {
				MessageQueueEntry messageQueueEntry = importResult.convertedFileException.createMessageQueueEntry(file,
		...
		if (entry != null) {
			...
			while ((otherEntry = root.getEntry(renamedFileName)) != null
					&& (renamedFile != null && StringUtility.isSpecified(renamedFile.getName()))) {
				renamedFile = renamedFile.getParentFile();
				if (renamedFile != null && StringUtility.isSpecified(renamedFile.getName())) {
					parentPath = parentPath + "_" + renamedFile.getName();
					renamedFileName = fileNameWithoutExtension + parentPath + fileExtension;
				}
			}
			if (otherEntry != null) {
				// Still no success, start enumerating.
				int number = 1;
				renamedFileName = fileNameWithoutExtension + "_" + number + fileExtension;
				while ((otherEntry = root.getEntry(renamedFileName)) != null && renamedFile != null) {
					number++;
					renamedFileName = fileNameWithoutExtension + "_" + number + fileExtension;
				}
			}
		...
		if (file.exists() && file.isFile()) {
			if (fileSizeLong == 0) {
				...
				messageQueue.sendMessage(entry, WorkbookEntry.Attributes.FILE_PATH, Messages.S419,
			...
			if (fileSizeLong > ENTRY_MAX_FILE_SIZE) {
				...
				// {2}.
				messageQueue.sendMessage(entry, WorkbookEntry.Attributes.FILE_PATH, com.wudsn.tools.base.Messages.E214,
						file.getAbsolutePath(), TextUtility.formatAsMemorySize(fileSizeLong),
		...
		int requiredBanks = (fileSize + root.getBankSize() - 1) / root.getBankSize();
	 ...
	 * @param importResult
	 *            The import result from the auto conversion, not <code>null</code>.
	 * @param content
	 *            The file content of the file associated to the entry, not
	 *            <code>null</code>.
	 * @param callback
	 *            The callback to confirm user interactions, not <code>null</code>.
	 * @return <code>true</code> if the processing can continue, <code>false</code>
	 *         if the processing was cancelled by the user.
	 */
	private boolean applyEntryDefaults(WorkbookEntry entry, ImportResult importResult, byte[] content,
		...
		if (cartridgeDatabaseEntry == null) {
			List<CartridgeDatabaseEntry> cartridgeDatabaseEntries = cartridgeDatabase
					.getEntriesBySizeAndCRC32(content.length / KB, entry.getContentCRC32());
		...
		if (cartridgeDatabaseEntry != null && !entry.getTitle().equals(cartridgeDatabaseEntry.getTitle())) {

			// Check if title is compatible.
			MessageQueue messageQueue = new MessageQueue();
			RepositoryValidation rv = RepositoryValidation.createInstance(messageQueue);
			rv.isStringValid(cartridgeDatabaseEntry, WorkbookEntry.Attributes.TITLE, cartridgeDatabaseEntry.getTitle());
			if (!messageQueue.containsError()) {

				// If title is compatible, apply the default title from the
				// database.
				entry.setContentType(ContentType
						.getInstanceByCartridgeType(cartridgeDatabaseEntry.getCartridgeType().getNumericId()));
				int result = callback.confirmUseTitleFromCartridgeDatabase(entry.getFileName(),
		...
		for (ContentType contentType : contentTypes) {
			...
			if (contentSizeInKB == 0 || entry.getContentSize() == contentSizeInKB * KB) {
		...
		if (entry.getContentType().equals(ContentType.FILE_ATR)) {
			AtrLoader.setDefaultParameters(entry, (AtrFileMenu) importableMenu, content);
		}
		return true;
	}
	 ...
	 * 
	 * @param indexes
	 *            The array of entry indexes, may be empty, not <code>null</code>.
	 * @param messageQueue
	 *            The message queue, not <code>null</code>.
	 */
		...
		if (rootFile != null && !rootFile.equals(file)) {
			for (WorkbookEntry entry : root.getUnmodifiableEntriesList()) {
				if (StringUtility.isEmpty(entry.getFilePath())) {
					entry.setFilePath(new File(getDataFolderPath(), entry.getFileName()).getAbsolutePath());
				}
			}
		}
	...
	private void saveEntryFilesToDataFolder(MessageQueue messageQueue) {
		...
		if (!dataFolderFile.exists()) {
			if (!dataFolderFile.mkdirs()) {
				...
				messageQueue.sendMessage(root, null, com.wudsn.tools.base.Messages.E203, dataFolderPath);
				return;
			}
		}
		...
		for (int i = 0; i < totalNumber; i++) {
			...
			try {
				...
				if (number % steps == 0) {
					...
					messageQueue.sendMessage(fileToCopy.entry, WorkbookEntry.Attributes.FILE_PATH, Messages.S417,
							TextUtility.formatAsDecimal(number), TextUtility.formatAsDecimal(totalNumber),
							TextUtility.formatAsDecimalPercent(number, totalNumber));
				}
				...
				messageQueue
						.sendMessage(ex.createMessageQueueEntry(fileToCopy.entry, WorkbookEntry.Attributes.FILE_PATH));
			}
		}
		...
		for (int i = 0; i < totalNumber; i++) {
			...
			int number = i + 1;
			if (number % 10 == 0) {
				messageQueue.sendMessage(file, WorkbookEntry.Attributes.FILE_PATH, Messages.S418,
						TextUtility.formatAsDecimal(number), TextUtility.formatAsDecimal(totalNumber),
						TextUtility.formatAsDecimalPercent(number, totalNumber));
			}
		...
		for (ReservedContentProvider reservedContentProvider : root.getReservedContentProviders()) {
			...
			if (reservedContentProvider.getRequiredBanksCount() > 0) {
				if (reservedContentProvider.getRequiredBankSize() > 0
						&& reservedContentProvider.getRequiredBankSize() != root.getBankSize()) {
					// ERROR: Reserved content provider '{0}' requires a bank
					// size of {1}.
					messageQueue.sendMessage(reservedContentProvider, null, Messages.E407,
							reservedContentProvider.getTitle(),
							TextUtility.formatAsMemorySize(reservedContentProvider.getRequiredBankSize()));
					continue;
				}
				...
				if (endBank > root.getBankCount()) {
					...
					messageQueue.sendMessage(reservedContentProvider, null, Messages.E408,
							reservedContentProvider.getTitle(),
							TextUtility.formatAsDecimal(reservedContentProvider.getRequiredBanksCount()),
							TextUtility.formatAsDecimal(endBank), TextUtility.formatAsDecimal(root.getBankCount()));
					continue;
				...
				for (int bankNumber = reservedContentProvider
					...
					if (otherReservedContentProvider == null) {
						...
						// content provider '{2}'.
						messageQueue.sendMessage(reservedContentProvider, null, Messages.E409,
								TextUtility.formatAsDecimal(bankNumber), otherReservedContentProvider.getTitle(),
								reservedContentProvider.getTitle());
					}
				}
			}
	...
	private void assignCurrentBanks(MessageQueue messageQueue) {
		...
		for (WorkbookEntry entry : root.getUnmodifiableEntriesList()) {
			...
			if (startBank >= 0) {
				if (rv.isLongValid(entry, WorkbookEntry.Attributes.START_BANK, 0, root.getBankCount() - 1, startBank)) {
	...
	public void assignNewBanks(MessageQueue messageQueue) {
		...
		List<WorkbookEntry> entries = new ArrayList<WorkbookEntry>(root.getUnmodifiableEntriesList());
		Collections.sort(entries, new Comparator<WorkbookEntry>() {
			...
			public int compare(WorkbookEntry o1, WorkbookEntry o2) {
				...
				result = JDK.Boolean.compare(o2.isStartBankFixed(), o1.isStartBankFixed());
				if (result == 0) {
	...
	/**
	 * Finds a free sequence of banks which either starts the fixed bank specified
	 * by the user or at properly aligned free space.
	 * 
	 * @param entry
	 *            The entry, not <code>null</code>.
	 * @param messageQueue
	 *            The message queue, not <code>null</code>.
	 * @return <code>true</code> if the entry is empty or if the entry is not empty
	 *         and was assigned properly, <code>false</code> otherwise.
	 */
	private boolean assignNewBanks(WorkbookEntry entry, MessageQueue messageQueue) {
		...
		if (entry.isStartBankFixed()) {
			RepositoryValidation rv = RepositoryValidation.createInstance(messageQueue);
			if (rv.isLongValid(entry, WorkbookEntry.Attributes.START_BANK, 0, root.getBankCount() - 1,
			...
			entry.setStartBankNumber(WorkbookEntry.START_BANK_UNDEFINED);
			bankFound = false;
			for (int bankNumber = 0; !bankFound && bankNumber < root.getBankCount(); bankNumber++) {
	 ...
	 *            The entry, not <code>null</code>.
	 * @param startBank
	 *            The start bank, a non-negative integer. Normally the current start
	 *            bank of the entry, except during re-assignment of to new banks.
	...
	public boolean assignBanks(WorkbookEntry entry, int startBank) {
		...
		if (entry.areBanksAssigned()) {
			throw new IllegalArgumentException("Entry for file '" + entry.getFileName() + "' starting at bank "
					+ entry.getStartBankNumber() + " already has banks assigned.");
		}
	...
	public void unassignBanks(WorkbookEntry entry) {
		...
		List<WorkbookBank> banks = root.getBanksList();
		for (int bankNumber = entry.getStartBankNumber(); bankNumber < entry.getStartBankNumber()
				+ entry.getRequiredBanksCount() && bankNumber < root.getBankCount(); bankNumber++) {
	...
	private String getDataFolderPath() {
		...
		result = result.substring(0, index) + DATA_FOLDER_FILE_EXTENSION + File.separator;
		return result;
	}
	...
	public WorkbookExport export(int exportFormat, MessageQueue messageQueue) {
		...
		for (int bankNumber = 0; bankNumber < root.getBankCount(); bankNumber++) {
			...
			if (reservedContentProvider != null && reservedContentProvider.getStartBankNumber() == bankNumber) {
				byte[] reservedContent = reservedContentProvider.createContent(this, result, messageQueue);
				if (reservedContent != null) {
					...
					if (reservedContent.length <= expectedLength) {
						...
						throw new RuntimeException("Reserved content provider '" + reservedContentProvider
								+ "' returned too much content. Expected " + expectedLength + " bytes and found "
				...
				if (!entries.isEmpty()) {
					...
					byte[] entryContent = getEntryFileContent(entry, messageQueue);
					...
					if (entryContent != null) {
						...
						if (entry.getContentType().equals(ContentType.FILE_ATR)) {
							try {
								...
								throw new RuntimeException("Entry '" + entry.getTitle() + "' with file path "
										+ entry.getFilePath() + " is no valid ATR file", ex);
							}
						}
		...
		case ExportFormat.CAR_IMAGE:
			...
			if (cartridgeType == CartridgeType.UNKNOWN) {
				...
				messageQueue.sendMessage(data, null, Messages.E404, root.getFlashTargetType().getText());
			}
			break;
		case ExportFormat.ATR_IMAGE:
			if (data.length % AtrFile.SECTOR_SIZE_8K != 0) {
				// ERROR: Size {0} of content is not a multiple of {1}.
				messageQueue.sendMessage(data, null, Messages.E410, TextUtility.formatAsMemorySize(data.length),
	...
	public byte[] getEntryFileContent(WorkbookEntry entry, MessageQueue messageQueue) {
		...
		if (!entryFile.exists()) {
			...
			messageQueue.sendMessage(entry, WorkbookEntry.Attributes.FILE_PATH, com.wudsn.tools.base.Messages.E203,
					filePath);
			return null;
		}
		...
		if (entryFileSize != entry.getFileSize()) {
			...
			messageQueue.sendMessage(entry, WorkbookEntry.Attributes.FILE_PATH, Messages.E403, filePath,
					TextUtility.formatAsMemorySize(entryFileSize), TextUtility.formatAsMemorySize(entry.getFileSize()));
			...
			if (cartridgeType != CartridgeType.UNKNOWN && entry.getContentSize() != cartridgeTypeSize) {
				...
				messageQueue.sendMessage(entry, WorkbookEntry.Attributes.CONTENT_TYPE, Messages.E413, filePath,
						TextUtility.formatAsMemorySize(entry.getContentSize()), entry.getContentType().getText(),
				...
				try {
					...
					byte[] entryContent = FileUtility.readBytes(filePath, inputStream, ENTRY_MAX_FILE_SIZE, true);
					...
					if (inputStream != null) {
						try {
							...
							messageQueue
									.sendMessage(ex.createMessageQueueEntry(entry, WorkbookEntry.Attributes.FILE_PATH));
						}
					}
				}
			}

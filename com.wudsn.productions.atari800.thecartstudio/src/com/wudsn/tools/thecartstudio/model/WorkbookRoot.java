...
public final class WorkbookRoot {
	...
	public static final class Attributes {
		...
		public static final Attribute FLASH_TARGET_TYPE = new Attribute("flashTargetType",
				DataTypes.WorkbookRoot_FlashTargetType);
		...
		public static final Attribute CARTRDIGE_TYPE = new Attribute("cartridgeType",
				DataTypes.WorkbookRoot_CartridgeType);
		public static final Attribute CARTRDIGE_MENU_TYPE = new Attribute("cartridgeMenuType",
				DataTypes.WorkbookRoot_CartridgeMenuType);
		public static final Attribute USER_SPACE_SIZE = new Attribute("userSpaceSize",
	...
	WorkbookRoot() {
		...
		setFlashTargetType(FlashTargetType.THECART_128MB, CartridgeType.CARTRIDGE_THECART_128M, 0, 0);
	...
	public void setFlashTargetType(FlashTargetType flashTargetType, CartridgeType cartridgeType, int bankCount,
			int bankSize) {
	 ...
	 * 
	 * @return The complete image size which is always a multiple of the bank size.
	 */
	 ...
	 * 
	 * @return The modifiable list of genres, may be empty, not <code>null</code>.
	 */
	...
	final List<ReservedContentProvider> getReservedContentProviders() {
		List<ReservedContentProvider> result = new ArrayList<ReservedContentProvider>(4);
	...
	/**
	 * Atomic option, does not fire mode change. This must be done by the caller.
	 * 
	 * @param index
	...
	public boolean contentEquals(WorkbookRoot other) {
		...
		if (!(other.unmodifiableGenresList.size() == unmodifiableGenresList.size())) {
			return false;
		}
		for (int i = 0; i < other.unmodifiableGenresList.size(); i++) {
			if (!(other.unmodifiableGenresList.get(i).equals(unmodifiableGenresList.get(i)))) {
				return false;
			}
		}
		if (!(other.unmodifiableEntriesList.size() == unmodifiableEntriesList.size())) {
			return false;
		}
		for (int i = 0; i < other.unmodifiableEntriesList.size(); i++) {
			if (!(other.unmodifiableEntriesList.get(i).equals(unmodifiableEntriesList.get(i)))) {
				return false;
			}
		}
	...
	final void serialize(Element element) {
		...
		Attributes.FLASH_TARGET_TYPE.serializeValueSet(element, flashTargetType);
		...
		Attributes.CARTRDIGE_MENU_TYPE.serializeValueSet(element, cartridgeMenuType);
	...
	final void deserialize(org.xml.sax.Attributes attributes) throws SAXException {
		...
		FlashTargetType flashTargetType = Attributes.FLASH_TARGET_TYPE.deserializeValueSet(attributes,
				FlashTargetType.class);
		if (flashTargetType == null) {
			flashTargetType = this.flashTargetType;
		}
		CartridgeType cartridgeType = Attributes.CARTRDIGE_TYPE.deserializeValueSet(attributes, CartridgeType.class);
		if (cartridgeType == null) {
		...
		cartridgeMenuType = Attributes.CARTRDIGE_MENU_TYPE.deserializeValueSet(attributes, CartridgeMenuType.class);

		userSpaceSize = Attributes.USER_SPACE_SIZE.deserializeInteger(attributes);
	}

	@Override
	public String toString() {
		return "title=" + title + " flashTargetType=" + flashTargetType + " bankCount=" + bankCount + " bankSize="
				+ bankSize + " cartridgeMenuType=" + cartridgeMenuType + " userSpaceSize=" + userSpaceSize
				+ " genresList=" + unmodifiableGenresList + " entriesList=" + unmodifiableEntriesList;
	}
}

...
public final class WorkbookOptionsDialog extends ModalDialog {
	...
	public WorkbookOptionsDialog(JFrame parent, Preferences preferences) {
		...
		titleTextField = SpringUtilities.createTextField(fieldsPane, DataTypes.WorkbookRoot_Title);
		flashTargetTypeField = SpringUtilities.createValueSetField(fieldsPane, DataTypes.WorkbookRoot_FlashTargetType,
		...
		cartridgeTypeField = SpringUtilities.createValueSetField(fieldsPane, DataTypes.WorkbookRoot_CartridgeType,
				CartridgeType.class);
		bankCountField = SpringUtilities.createIntegerField(fieldsPane, DataTypes.WorkbookRoot_BankCount);
		bankSizeField = SpringUtilities.createMemorySizeField(fieldsPane, DataTypes.WorkbookRoot_BankSize);

		cartridgeMenuTypeField = SpringUtilities.createValueSetField(fieldsPane,
				DataTypes.WorkbookRoot_CartridgeMenuType, CartridgeMenuType.class);
		userSpaceSizeField = SpringUtilities.createMemorySizeField(fieldsPane, DataTypes.WorkbookRoot_UserSpaceSize);
		genreNamesField = SpringUtilities.createMemorySizeField(fieldsPane, DataTypes.WorkbookRoot_GenreNames);
	}
	...
	public boolean showModal(Workbook workbook, MessageQueue messageQueue) {
		...
		okPressed = false;
		// Lay out the panel.
		int stringWidth = getParent().getGraphics().getFontMetrics().stringWidth("a")
				* WorkbookRoot.Attributes.TITLE.getDataType().getMaximumLength();
		Dimension size = titleTextField.getPreferredSize();
		titleTextField.setPreferredSize(new Dimension(stringWidth, size.height));
		...
		if (okPressed) {
			...
			boolean contentStructureEquals = workbook.getRoot().contentEquals(root);
			if (!contentStructureEquals) {
				workbook.getRoot().setFlashTargetType(root.getFlashTargetType(), root.getCartridgeType(),
						root.getBankCount(), root.getBankSize());
				workbook.getRoot().setCartridgeMenuType(root.getCartridgeMenuType());
	...
	protected void dataFromUi() {
		...
		root.setFlashTargetType(flashTargetTypeField.getValue(), cartridgeTypeField.getValue(),
				bankCountField.getValue(), bankSizeField.getValue());
	...
	protected void dataToUi() {
		...
		boolean editable = root.getFlashTargetType().equals(FlashTargetType.USER_DEFINED);
		...
		editable = (root.getFlashTargetType().getSupportedCartridgeMenuTypes().size() > 1);
		...
		if (!root.getFlashTargetType().isCartridgeMenuTypeSupported(cartridgeMenuType)) {
	...
	void performFlashTargetTypeChanged() {
		...
		if (!flashTargetType.isCartridgeMenuTypeSupported(root.getCartridgeMenuType())) {
			root.setCartridgeMenuType(flashTargetType.getSupportedCartridgeMenuTypes().get(0));
		}
		dataToUi();
	}
	...
	protected boolean validateOK() {
		...
		if (messageQueue.containsError()) {
			StandardDialog.showErrorMessage(this, messageQueue.getFirstError().getMessageText(), getTitle());
			return false;
		}
		return true;
	}

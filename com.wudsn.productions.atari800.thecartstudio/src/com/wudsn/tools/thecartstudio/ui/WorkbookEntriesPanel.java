...
public final class WorkbookEntriesPanel extends AttributeTablePanel {
	...
	public final static class TableModel extends AttributeTableModel {
		...
		public TableModel(final Workbook workbook) {
			...
			addColumn(Attributes.TITLE, Column.FIXED | Column.SORTABLE, StringUtility.CASE_INSENSITIVE_COMPARATOR, null,
					null);

			genreNamesList = Collections.emptyList();
			genreComboBoxModel = new DefaultComboBoxModel<String>();
			JComboBox<String> genreComboBox = new JComboBox<String>(genreComboBoxModel);
			...
			addColumn(Attributes.FILE_HEADER_TYPE, Column.HIDDEN | Column.SORTABLE);
			addColumn(Attributes.CONTENT_SIZE, Column.HIDDEN | Column.SORTABLE);
			addColumn(Attributes.CONTENT_CRC32, Column.HIDDEN | Column.SORTABLE, null, new DefaultTableCellRenderer() {
				@Override
				protected void setValue(Object value) {
					...
					if (crc32 == 0) {
						...
						setText(Long.toHexString(crc32 & 0xffffffffl).toUpperCase());
					}
					setFont(monoSpacedFont);
				}
			}, null);
			addColumn(Attributes.CONTENT_TYPE, Column.VISIBLE | Column.SORTABLE, null, null,
					new DefaultCellEditor(new ValueSetField<ContentType>(ContentType.class)) {
						@Override
						public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected,
								int viewRowIndex, int viewColumnIndex) {
							Component component = super.getTableCellEditorComponent(table, value, isSelected,
							...
							comboBox.setModel(ContentTypeComboxBoxModelFactory.getModel(filterByFileContentSize,
									fileContentSize, root.getFlashTargetType(), currentValue));
							comboBox.setSelectedItem(currentValue);
							return component;
						}
					});
			addColumn(Attributes.DISPLAY_MODE, Column.VISIBLE | Column.SORTABLE, null, null,
					new DefaultCellEditor(new ValueSetField<DisplayMode>(DisplayMode.class)));
			addColumn(Attributes.PARAMETERS, Column.HIDDEN | Column.SORTABLE);
			addColumn(Attributes.START_BANK_FIXED_INDICATOR, Column.HIDDEN | Column.SORTABLE);
			addColumn(Attributes.START_BANK, Column.HIDDEN | Column.SORTABLE, null, new DefaultTableCellRenderer() {
				@Override
				protected void setValue(Object value) {
			...
			addColumn(Attributes.REQUIRED_BANKS_COUNT, Column.HIDDEN | Column.SORTABLE);
			addColumn(Attributes.BANKS_ASSIGNED_INDICATOR, Column.VISIBLE | Column.SORTABLE);

		}

		public void updateFromWorkbook() {
			List<String> genreNamesList = workbook.getRoot().getSortedGenreNamesList();
		...
		public boolean isCellEditable(int row, int column) {
			...
			case Columns.CONTENT_TYPE:
				...
				return (entry.getType().equals(WorkbookEntryType.FILE_ENTRY)
						...
						|| (entry.getType().equals(WorkbookEntryType.USER_SPACE_ENTRY));
		...
		public void setValueAt(Object value, int row, int column) {
			try {
				...
				case Columns.PARAMETERS:
					...
					break;
				case Columns.START_BANK_FIXED:
					entry.setStartBankFixedIndicator(((Boolean) value).booleanValue());
					break;
				case Columns.START_BANK:
	...
	public void removeSelectedWorkbookEntries(MessageQueue messageQueue) {
		...
		for (int i = 0; i < selectedViewRowIndexes.length; i++) {
			selectedModelRowIndexes[i] = table.convertRowIndexToModel(selectedViewRowIndexes[i]);
		}
	...
	public void setSelectedWorkbookEntriesGenre(String genreName, MessageQueue messageQueue) {
		...
		workbook.setEntriesGenreName(genreName, selectedModelRowIndexes, messageQueue);
	}
